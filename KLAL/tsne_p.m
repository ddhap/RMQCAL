...
% University of California, San Diego
    ...
    momentum = 0.5;                                     % initial momentum   a(t)
    final_momentum = 0.8;                               % value to which momentum is changed
    mom_switch_iter = 250;                              % iteration at which momentum is changed
    stop_lying_iter = 100;                              % iteration at which lying about P-values is stopped
    max_iter = 1000;                                    % maximum number of iterations
    epsilon = 500;                                      % initial learning rate 很想n的那个字符
    min_gain = .01;                                     % minimum gain for delta-bar-delta
    
    % Make sure P-vals are set properly
    P(1:n + 1:end) = 0;                                 % set diagonal to zero 设置对角为0
    P = 0.5 * (P + P');                                 % symmetrize P-values 确保其对称
    P = max(P ./ sum(P(:)), realmin);                   % make sure P-values sum to one  realmin是最小的大于0的值 得到Pij
    ...
    if ~initial_solution
        ydata = .0001 * randn(n, no_dims);              %初始的Qij值
    end
    y_incs  = zeros(size(ydata));
    gains = ones(size(ydata));
    
    % Run the iterations
    for iter=1:max_iter     %每次迭代
        
        % Compute joint probability that point i and j are neighbors
        sum_ydata = sum(ydata .^ 2, 2);
        num = 1 ./ (1 + bsxfun(@plus, sum_ydata, bsxfun(@plus, sum_ydata', -2 * (ydata * ydata')))); % Student-t distribution num就是1+||yi-yj||^2的-1次方
