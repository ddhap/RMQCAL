function [aggR, pval, rowNames] = aggregateRanks_w(R, N, method,weight,complete,topCutoff)
...
elseif size(weight,2)~=1&&size(weight,1)~=1
   weight=ones(1,size(R,2));   
elseif size(weight,2)~=size(R,2)&&size(weight,1)~=size(R,2)
   weight=ones(1,size(R,2));  
...
weight_tmp=nan(size(rmat,1),size(rmat,2));
for ii=1:size(rmat,1)
    weight_tmp(ii,:)=weight;
end
rmat=weight_tmp.*rmat;
