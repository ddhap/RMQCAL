%% Iterative Partial Rank Aggregation Pivoting Algorithm
% As used in the manuscript "Differential thermotolerance among 110
% Symbiodinium phylotypes" by Swain et al.
% January 29, 2016

%% Clear workspace
clearvars; close all;

%% Load data from spreadsheet and populate a list of elements

% % Get the raw data from literature (stored in spreadsheet)

% Format of spreadsheet data: Each row in a unique input list ex.(X1<X2<X3)
% The leftmost column contains the ordered list with the element names
% separated by the '<' operator to desginate their relative order or commas
% to indicate ties. The middle column contains the number of unique
% categories or cohorts in the list and the rightmost column contains the
% number of elements in the list.

% read the input lists from the spreadsheet
[num0, txt0, raw0] = xlsread(...
    'Phylotype_Rankings.xlsx', 'Phylotype ranking', 'A3:C39');
...
[rank_val, rank_indx] = sort(average_rank);
final_rank = allElements(rank_indx);
uncertainty_rank = uncertainty_rank(rank_indx);
sterr_rank = sterr_rank(rank_indx);

%% Calculate performance metrics

% Calculate the Kendall Tau distance and coherence performance metrics of
% the final ranking relative to the input partial rankings

% initialize variables for metrics
KendallTau = zeros(i,1);
Coherence = zeros(i,1);
elementSequence = 1:totalElements;
element_idx = logical(element_idx);
% for each iteration
for kk = 1:i
    % for each report
    for jj = 1:num_reports
        % Get the input ranks
        x = all_cohorts(jj,element_idx(jj,:));
        [~, KT_rank_indx] = sort(x);
        x = elementSequence(element_idx(jj,:));
        x = x(KT_rank_indx);
        clear KT_rank_indx
        
        % Get the calculated ranks for each iteration
        y = iterative_ranks(kk,element_idx(jj,:));
        [~, KT_rank_indx] = sort(y);
        y = elementSequence(element_idx(jj,:));
        y = y(KT_rank_indx);
        % Calculate the KendallTau distance for each iteration
        k1=KendallDist(x,y);
        KendallTau(kk) = KendallTau(kk) +k1;
        N = length(x);
        % Calculate the Coherence
        Coherence(kk) = Coherence(kk) + N*(1-KendallDist(x,y)/(N*(N-1)/2));
        %clear x y N
    end
end

% Display if ranking converged, number of iterations, and run time
if i == n
    display(['Ranking did not converge, stopped after ', num2str(i),...
        ' iterations, in ' num2str(toc), ' seconds']);
else
    display(['Ranking Converged in ' num2str(i), ' iterations, in '...
        num2str(toc), ' seconds']);
end
